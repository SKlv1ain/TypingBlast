...
class GameOverView:
    def __init__(self, screen):
        ...
        self.back_button = pg.Rect(280, 500, 240, 60)  # Moved button lower
    ...
    def draw(self, stats):
        ...
        title_rect = title_text.get_rect(center=(self.screen.image.get_width() // 2, 80))
        shadow_rect = shadow.get_rect(center=(self.screen.image.get_width() // 2 + 1, 81))
        self.screen.image.blit(shadow, shadow_rect)
        self.screen.image.blit(title_text, title_rect)

        # Draw stat box (moved lower)
        box_rect = pg.Rect(150, 150, 500, 300)
